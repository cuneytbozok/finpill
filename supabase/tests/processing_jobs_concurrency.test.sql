...
do $do$ begin perform extensions.dblink_exec('worker_a', $$
  select public.enqueue_job('market_valuation_refresh', 'market_valuation_refresh:conc1:valuation_v1')
$$); end $do$;
...
do $do$ begin perform extensions.dblink_exec('worker_a', 'begin'); end $do$;
do $do$ begin perform extensions.dblink_exec('worker_a', $$
  select public.enqueue_job('kap_disclosure_detail', 'kap_disclosure_detail:7000003:fetch_v1')
$$); end $do$;
...
do $do$ begin perform extensions.dblink_exec('worker_a', $$
  select public.enqueue_job('market_valuation_refresh', 'market_valuation_refresh:conc3:valuation_v1')
$$); end $do$;
do $do$ begin perform extensions.dblink_exec('worker_a', $$
  select * from public.claim_jobs('worker-a', array['market_valuation_refresh'], 5, 60)
$$); end $do$;
...
do $do$ begin perform extensions.dblink_exec('worker_a', 'begin'); end $do$;
do $do$ begin perform extensions.dblink_exec('worker_a', $$
  select public.assert_job_lease(
...
select throws_ok(
  $$select extensions.dblink_exec('worker_a', $q$
