...
- An explicit account-level pilot eligibility grant and a browser-issued bearer token are still needed to prove eligible Data API CRUD and the invited account's API profile flow. Cross-user denial has been exercised with two simulated subjects in the rollback-only SQL probe, not with two live Clerk accounts.
