...
- Unfinished acceptance: Docker is unavailable on this host, so `npm run db:test` and the new pgTAP file were not run locally. Two-live-account isolation and the browser-issued token's `/api/v1/profile` path remain unverified. A02/A03 are not accepted.
- Next action: complete owner-controlled browser sign-in for the local API acceptance path; run disposable database CI replay when Docker is available. Review and merge the task PR only after the required acceptance is recorded. Stop after this task's handoff rather than selecting 01.07 in this session.
