...
| 01.04 | complete | [PR #18](https://github.com/cuneytbozok/finpill/pull/18) merged as `82420d9`; CI quality/database and Vercel Preview passed. Signed Release device lifecycle evidence is in its handoff. |
| 01.05 | complete | [PR #19](https://github.com/cuneytbozok/finpill/pull/19) merged as `026e3c5`; Android SDK bridge and invited-account Release emulator lifecycle are in its handoff. |
...
| 01.09 | in_progress | Draft [PR #23](https://github.com/cuneytbozok/finpill/pull/23); deployed API v1/CORS and API-enabled unsigned native staging builds passed. Preview-to-pilot isolation remains unproven because pilot resources are not registered. See handoff. |
| 01.10 | pending | — |
...
- Owner: Codex; branch `codex/01.04-ios-clerk-auth`, worktree `.worktrees/01.04`, base `dc21397` from fetched/pruned `origin/main`. The user's `main` checkout was not changed. Implementation and verification through `87b16ca`; [PR #18](https://github.com/cuneytbozok/finpill/pull/18) is ready for review.
- Deliverables: ClerkKit and ClerkKitUI 1.3.9 pinned in the Xcode App target; a local Capacitor bridge for SDK configuration, native sign-in UI, state, sign-out, and fresh session tokens; iOS `AuthPort` wiring, account controls, and session token readiness in the shared static client; [iOS setup and acceptance guide](IOS_AUTH.md). Clerk owns Keychain persistence. No API, database, web-auth, Android-auth, or financial contract was changed. The SDK requires iOS 17+, so the App target and generated Capacitor package minimum changed from iOS 15 to 17. Codex did not edit signing settings; the owner selected the Personal Team in Xcode.
- Verification passed: pinned Node 24.21.0/npm 11.19.0 clean offline install; `npm run check` with the ignored development publishable-key configuration (Prettier, zero-warning lint, typecheck, 99 tests, client/API builds); `npm run native:sync` with 43 matching static assets; Xcode package resolution and unsigned iOS Release simulator build against the booted iPhone 17 Pro / iOS 26.5; `npm run test:environment-build` isolated from ignored local settings, including invalid environment cases, production liveness and a 43-file static scan. Xcode project syntax and `git diff --check` passed. The Clerk package resolved to tag 1.3.9/revision `32f7e0f`; its transitive packages are locked in `Package.resolved`.
- Native configuration and partial UI evidence: the owner showed the development Native API enabled, iOS app registration for App ID Prefix `7NA7D47449`/bundle ID `com.cuneytbozok.finpill`, and the callback URL allowlisted. An auth-enabled Release build launched in the iPhone 17 Pro simulator and opened Clerk's native sheet; its Close control dismissed the sheet. The sheet had no input fields. Simulator logs reported Keychain OSStatus `-34018` (missing entitlement) and a `native_api_disabled` response from the SDK's fallback `clerk.clerk.dev` endpoint, not from the configured `ample-chicken-233` instance. The unsigned simulator artifact has no TeamIdentifier or Keychain entitlement, so this is not evidence that the development instance's Native API is disabled. The adapter was updated to wait for SDK readiness and show an error after startup failure; `npm run check`, auth-enabled `npm run native:sync`, and the iOS Release simulator build passed again.
- Signed device acceptance: a Release `iphoneos` build passed with automatic Personal Team provisioning for the owner's iPhone 15 Pro. The installed app's signature has TeamIdentifier `7NA7D47449` and application identifier `7NA7D47449.com.cuneytbozok.finpill`. The development Clerk instance rendered the full native sign-in sheet; the owner entered an invited account's credentials and reached signed-in Finpill. A force-terminated cold restart restored the session without another sign-in. `AuthPort.getAccessToken()` reported “Oturum hazır” after initial sign-in and on a later periodic request beyond the pinned SDK's documented one-minute token TTL; no token was displayed. The owner left the app in the background for one minute and confirmed it resumed to “Oturum hazır” without sign-in. With Airplane Mode on for about 90 seconds, the app showed “Oturum yenilenemedi”; after reconnection it returned to “Oturum hazır” without sign-in. The owner confirmed “Çıkış yap” returned to sign-in, and opening then dismissing the sign-in sheet kept the app signed out. This checks expiry-boundary token acquisition and network recovery; it does not claim a decoded JWT expiry or protected API acceptance.
- Leakage and final checks: the native bridge returns a token only to `AuthPort`; source inspection found no WebView local/session storage, cookie, or token logging path in the iOS adapter and account view. The signed app's 108 files were scanned against the development Clerk server key from an ignored local environment file; no match occurred and no key was printed. `npm run check` passed Prettier, zero-warning lint, typecheck, 99 tests, and client/API builds after the status UI change. `npm run test:environment-build` passed invalid-config/runtime, production liveness, and a 43-file static scan with the ignored client environment temporarily removed and restored. Auth-enabled `npm run native:sync` and signed Release build/install passed for the status UI change. The earlier unsigned simulator's missing Keychain entitlement remains a simulator limitation, not a device failure. No A01–A03 gate is accepted.
- Next action: review and merge PR #18; native bearer acceptance against the protected API remains task 01.10. Preserve the 01.02 iOS offline waiver as waived, not passed. Do not start another roadmap task in this implementation session.

## Task 01.05 Android SDK adapter handoff — 2026-09-23

- Owner: Codex; branch `codex/01.05-android-clerk-auth`, worktree `.worktrees/01.05`, base `82420d9` from fetched/pruned `origin/main`. GitHub confirms task 01.04 [PR #18](https://github.com/cuneytbozok/finpill/pull/18) merged with successful quality, database and Vercel Preview checks; the ledger's prior `in_review` state was stale. The user's `main` checkout was not changed.
- Deliverables: pinned Clerk Android UI SDK 1.0.39, local Capacitor bridge for SDK initialization, native sign-in UI, state, sign-out and session tokens; Android `AuthPort` wiring through the existing shared client account controls; [Android setup and acceptance guide](ANDROID_AUTH.md). Clerk owns encrypted native persistence, and Android backup is disabled. No API, database, web-auth, iOS-auth, financial contract, or architecture gate changed.
- Static verification: pinned Node 24.21.0/npm 11.19.0 offline install; `npm run check` passed Prettier, zero-warning lint, strict typecheck, 99 tests, static client and API builds under explicit local disabled-integration settings. `npm run test:environment-build` passed invalid-config/runtime cases, production liveness, and a 43-file static scan after the ignored auth configuration was temporarily held and restored. An auth-enabled `npm run native:sync` produced 43 matching static assets for iOS and Android. The Kotlin/Java Android Release build passed with JDK 21, Gradle 8.14.3 and local development signing. APK Signature Scheme v2 verified, with package `com.cuneytbozok.finpill`, minimum SDK 24 and target SDK 36. A 1,217-file APK scan found zero server-key pattern matches; source inspection found no WebView storage, cookie or token logging path in the adapter.
- Emulator evidence: the auth-enabled Release APK installed and launched on the API 36 `Finpill_API_36` emulator. [Signed-out home](evidence/01.05/android-signed-out.png) displayed the sign-in control; tapping it opened the development instance's native Clerk [sign-in screen](evidence/01.05/android-clerk-sign-in.png) with an email field. Closing the screen returned to “Giriş yap.” A force-stop and cold relaunch again settled at “Giriş yap.” This verifies SDK initialization, native UI presentation, cancellation, and signed-out restart only.
- Invited-account Release acceptance: the owner privately entered an invited account in the visible API 36 emulator and reported “Oturum hazır”; Codex read only those status labels, not credentials, account details, or tokens. `uiautomator` confirmed “Oturum hazır” and “Çıkış yap.” A force-stop/cold relaunch restored the signed-in session and returned to “Oturum hazır,” showing process recreation and SDK-managed persistence. After 67 seconds in the background, resume showed “Oturum hazır”; this exercises token acquisition beyond the nominal one-minute interval without claiming a decoded JWT expiry. Emulator Airplane Mode produced “Oturum yenilenemedi” after about 70 seconds; after reconnection the next SDK request returned to “Oturum hazır” without sign-in. Tapping “Çıkış yap” returned to “Giriş yap”; a second force-stop/cold relaunch remained signed out. Opening and dismissing the native sheet before authentication had already left the shell signed out.
- Remaining review: GitHub quality and database checks and Vercel Preview passed for PR #19. Code review and merge remain. The Clerk dashboard's Android application registration was not independently inspected; the actual development-instance native sign-in succeeded on the locally signed emulator. Production-style identity configuration, native protected API bearer acceptance, and RLS proof remain task 01.10. A01–A03 are not accepted. Preserve task 01.02's waived iOS offline check as waived.
- Publication: implementation/evidence commit `acfa810` and prior ledger closeout `a1cc28d` are in [PR #19](https://github.com/cuneytbozok/finpill/pull/19); this live-acceptance update follows on the same branch. Next session: reconcile PR #19 review/merge. If it is merely awaiting review, re-run canonical discovery and select task 01.06, whose 01.03/00.05 dependencies are complete, subject to its hosted Supabase prerequisites. Do not start another task in this session.

## Task 01.06 Clerk/Supabase authorization handoff — 2026-09-23

- Owner: Codex; branch `codex/01.06-clerk-supabase-rls`, worktree `.worktrees/01.06`, base `026e3c5` from fetched/pruned `origin/main`. Implementation commit `7d1080f` is in [Draft PR #20](https://github.com/cuneytbozok/finpill/pull/20). GitHub confirmed 01.05 PR #19 merged; the prior ledger state was stale. The user's `main` checkout was not changed.
- Deliverables: pinned `@supabase/supabase-js` user-scoped API client using a verified Clerk bearer token and publishable key; `GET`/`POST`/`PATCH /api/v1/profile`; a migration for operator-managed pilot eligibility and minimal text-ID profiles with RLS and column grants; local Clerk third-party configuration, pgTAP policy tests, and [setup/evidence guide](USER_DATA_AUTH.md). No service-role authority is used for user requests. No financial, Storage, native, or product-screen scope was added.
- Hosted staging: the owner designated existing Supabase project `gsgkoiwjkqbkuyyafzbf` as staging. Before this task it had no application tables or migrations. Foundation and 01.06 migrations were applied. At the owner's explicit approval, the Clerk development instance's Supabase integration and the staging project's Clerk Third-Party Auth connection were enabled; both dashboards showed enabled. The project dashboard labels its default branch `main PRODUCTION`, so no separate pilot environment is claimed.
- Database verification: a rollback-only staging SQL probe simulated two authenticated Clerk subjects plus a disabled subject and anon role. It passed self-read, cross-user read/write denial, ownership-reassignment denial, eligibility-write denial, disabled-pilot denial, and anonymous denial. Staging security advisor returned no lints. Direct Data API requests without a token or with a malformed token returned `401`; real active Clerk tokens carried `role=authenticated` and could query profiles but saw zero rows. The invited account's direct profile insert returned `403` while pilot-ineligible. After explicit owner approval, the sole existing invited development account received staging pilot eligibility. Its live Clerk session reached the Data API: eligibility and profile reads returned `200`, profile creation `201`, profile update `200`, and an ownership-reassignment attempt `403`. No real account identifier, token, or credential was logged or committed.
- API and build verification: `npm run check` passed formatting, zero-warning lint, strict typecheck, 99 existing tests, static client export and API build under explicit local disabled-integration settings. An auth-and-staging-database-enabled API build passed separately. `npm run test:environment-build` passed invalid-config/runtime, production liveness, and 43-file static artifact scan. Local API returned `401` unsigned/invalid bearer and `403` untrusted origin. Backend-minted Clerk session tokens lack the `azp` claim required by the existing API verifier, so their `401` cannot establish browser-issued API acceptance.
- Preview verification: the initial PR preview failed because the static client Vercel project had two unused `NEXT_PUBLIC_SUPABASE_*` variables rejected by its public-variable allowlist. The owner explicitly approved permanently deleting those two from the client project; the ignored API `.env.local` retains its Supabase URL and publishable key. Redeployed PR preview `39HrfrLDhfGp3zsL4aDtgYdMzE2F` reached Ready and exported all static pages. This does not deploy the separate API.
- CI verification: PR #20 commit `fb7d869` passed GitHub quality (including `npm run check`, native sync, environment build, and clean diff), Docker-backed database (`npm run db:test` with the new pgTAP file), and Vercel Preview checks. Docker is unavailable on this host, so database replay was verified in CI rather than locally.
- Unfinished acceptance: two-live-account isolation and the browser-issued token's `/api/v1/profile` path remain unverified. A02/A03 are not accepted.
- Next action: complete owner-controlled browser sign-in for the local API acceptance path. Review and merge the task PR only after the required acceptance is recorded. Stop after this task's handoff rather than selecting 01.07 in this session.

## Task 01.07 shared deep-link handoff — 2026-09-23

- Discovery: fetched/pruned `origin`; PR #20 was already merged as `35b1a12` even though the canonical 01.06 row said `in_review`. The row is corrected above. Its browser-issued API and two-live-account checks remain unverified and A02/A03 remain unaccepted. Task 01.07 was selected next because 01.02 and 01.03 are merged. The user's `main` checkout was not changed.
- Owner: Codex; branch `codex/01.07-deep-links`, isolated worktree `.worktrees/01.07`, base `35b1a12` from `origin/main`. Implementation/evidence commit `2cfb586` is in [Draft PR #21](https://github.com/cuneytbozok/finpill/pull/21).
- Deliverables: pinned Capacitor App plugin; cold/warm URL listeners and Android Back behavior in the shared client; exact-origin and known-route link resolver with hostile-host and malformed-route rejection; iOS Associated Domains entitlement and AASA file; Android App Links manifest and Digital Asset Links file; public deep-link origin validation; [setup and acceptance guide](DEEP_LINKS.md). The owner proposed `https://finpill.vercel.app`, and a live HTTPS `200` response confirmed the host. The Android assetlinks fingerprint is for the owner's local debug keystore used by Release-mode emulator builds; distribution signing remains unselected.
- Verification: `npm run check` passed on Node 24.21.0/npm 11.19.0 with explicit local disabled-integration settings: Prettier, zero-warning lint, strict typecheck, 116 tests, static client export and API build. `npm run test:environment-build` passed invalid-config/runtime checks, production liveness and a 47-file static scan. Auth-disabled native sync copied all 47 export assets and the App plugin to iOS/Android; `npm run native:inspect` passed. An unsigned iOS Release simulator build passed. Android Release with local signing, JDK 21 and SDK 36 passed; the compiled APK manifest contains the verified HTTPS filters. Entitlements and Xcode project syntax, association JSON and `git diff --check` passed.
- CI and Preview: [PR #21](https://github.com/cuneytbozok/finpill/pull/21) commit `795d783` passed GitHub quality and Docker-backed database jobs plus the Vercel Preview check. The preview deployment is Ready, but a direct association-file request redirects to Vercel SSO; it is not public-file or OS association evidence. Only a public response from the exact production host can establish that.
- iOS signing blocker found on 2026-09-24: Xcode was open on the older 01.04 worktree, with `Cuneyt Bozok (Personal Team)` selected. A signed Release build from the actual 01.07 worktree failed because that team's provisioning profile lacks Associated Domains and `com.apple.developer.associated-domains`. The entitlement file is now prepared but opt-in: its `CODE_SIGN_ENTITLEMENTS` setting was removed from the App target so current Personal Team builds continue to work. A signed iPhoneOS Release build then passed. This does not prove Universal Links. Apple Developer Program team access and a profile with the capability are required before enabling and testing the entitlement.
- Remaining acceptance: association files are not deployed to `finpill.vercel.app`, so OS verification is not claimed. Signed-device cold/warm company/tab links, interrupted sign-in return, browser/Android Back and malformed/untrusted native URL handling need live checks after deployment. iOS additionally needs a capable signing team and entitlement enabled. A local Android debug fingerprint is not a distribution signing decision. Task 01.07 remains `in_review` until these checks are recorded. Do not accept A01–A03 from static or unsigned evidence.
- Next action: review the task PR; obtain Apple Developer Program team access and enable Associated Domains on the 01.07 App target; deploy association files to the exact host; then perform the guide's installed-device acceptance. After this task is accepted and merged, re-run canonical discovery before selecting 01.09. Stop after this handoff in this implementation session.

## Task 01.07 owner acceptance and merge closeout — 2026-09-24

- Verified fetched/pruned `origin/main` at merge commit `288db6369114c0330a94cbcdc6eaab6560a3fb31`; GitHub confirms [PR #21](https://github.com/cuneytbozok/finpill/pull/21) merged with quality, Docker-backed database and Vercel Preview checks successful on head `2aed0e5`. The user's `main` checkout was not modified. This documentation closeout uses branch `codex/01.07-closeout` from that verified merge; commit `fb2ba89` is in [PR #22](https://github.com/cuneytbozok/finpill/pull/22).
- Owner decision: the user accepted and merged the PR, explicitly stated that external links opening native apps must not block roadmap completion, and plans to obtain an official domain and Apple Developer Program membership after the rest of the roadmap works. The provisional `finpill.vercel.app` host and local Android signer are not final link identities.
- **Accepted exception, not a passing test:** OS-verified HTTPS Universal/App Link opening on iOS and Android was not performed. After merge, the `apple-app-site-association` and `assetlinks.json` files under `https://finpill.vercel.app/.well-known/` returned public `200` JSON without redirects and SHA-256 hashes matching the tracked files (`83b8f3f0a5024b1c9edb9ba7f7df8885f2ff1dcdcec2409fa4fe9e6112d9e9de` and `b3f674200a42acdaf9904f2a5a9a04cef1d9241ded4b2872d35f412f46cf185a`). This verifies file delivery only. Personal Team signing lacks the iOS Associated Domains entitlement; the prepared entitlement is inactive so signed private-use builds continue to work. No external native link-opening result is claimed, and no A01–A03 gate is accepted by this closeout.
...
- Hosted state: Chrome shows the existing Finpill client project, Ready Production at `finpill.vercel.app` from `c26804d`, and a Ready task-branch client Preview at `https://finpill-git-codex-0109-deploym-ef40e4-cuneyts-projects-ec980692.vercel.app`. Client Preview settings contain branch-scoped public `NEXT_PUBLIC_API_ENABLED=true` and the exact API Preview origin; after commit `1d87017` both projects redeployed Ready and the client shell loaded in authenticated Chrome. The API origin was not independently read from its built JavaScript, so the baked setting is inferred from the scoped Vercel configuration and fresh deployment rather than directly proved. Hosted client auth remains disabled. The separate `finpill-api` project (`prj_RPrp1YttJigzDOBXIUI3cYGbBjJb`) uses Root Directory `apps/api`, outside-root files and workspace install/build commands. API Preview has only `APP_ENV=staging` and exact client `CLIENT_ORIGINS`; API Production has no application variables or successful deployment. Its task-branch Preview at `https://finpill-api-git-codex-0109-dep-608ea4-cuneyts-projects-ec980692.vercel.app` is Ready. The Vercel connector returns 403 for the team, while authenticated Chrome is accessible. Draft [PR #23](https://github.com/cuneytbozok/finpill/pull/23) holds the task changes; all five checks passed on `1d87017`.
- Approval boundary: automatic approval review initially rejected removing Vercel Authentication from the task-branch API Preview domain because delegated technical judgment was insufficient for that security change. The owner then explicitly approved that exact domain exception, and Chrome verified it under Unprotected Domains; Production protection remains enabled. Automatic approval also rejected saving provisional API Production settings because pilot resources and Production origins have not been verified. Those settings remain unsaved.
- Live checks: public `GET /api/v1/health` returned `200` JSON with `apiVersion: "v1"`; allowed client-origin `OPTIONS /api/v1/session` returned `204` with the exact allow-origin and Authorization header; untrusted-origin preflight returned `403` without allow-origin; allowed-origin session GET without bearer returned `401`. The API Preview has no Clerk or Supabase application credentials, so no positive hosted bearer or data request is claimed.
- Native staging profile: `npm run native:release -- staging` against the public API v1 succeeded with the existing test Clerk publishable key, static export, iOS unsigned Release simulator and Android unsigned Release APK. Ignored manifest records revision `9234d4d`, API v1 and static index SHA-256 `6472fa3d6a3d27876899599a2274c78c5567f20c1eeec88da28f45710ccf1429`. `node tools/check-native-assets.mjs` verified 47 matching static files in both synced native projects and scanned forbidden server-secret patterns. Extracted iOS app matched all 47; Android's APK omitted only the two `.well-known` association files, and its 45 other static assets matched their source hashes. A scan of all 47 APK public-asset entries found no forbidden credential patterns. The unsigned builds were not device-installed or signed for distribution.
- Remaining task acceptance: resolve Preview-to-pilot isolation and independently prove the client Preview's baked API origin if required for strict acceptance. Only owner-designated staging Supabase project `gsgkoiwjkqbkuyyafzbf` is registered and no pilot/live Clerk environment exists. No Preview credential is currently configured on the API, but this is not a live negative pilot-access test. Owner disposition is needed if accepting that explicit evidence gap for task 01.09; otherwise establish pilot resources and test the boundary. Keep A01–A03 draft and do not select 01.10 yet.
