...
- Remaining acceptance: association files are not deployed to `finpill.vercel.app`, so OS verification is not claimed. Signed-device cold/warm company/tab links, interrupted sign-in return, browser/Android Back, malformed/untrusted native URL handling and iOS provisioning capability need live checks after deployment. A local Android debug fingerprint is not a distribution signing decision. Task 01.07 remains `in_review` until these checks are recorded. Do not accept A01–A03 from static or unsigned evidence.
- Next action: review the task PR and deploy the association files to the exact host, then perform the guide's installed-device acceptance. After this task is accepted and merged, re-run canonical discovery before selecting 01.09. Stop after this handoff in this implementation session.
