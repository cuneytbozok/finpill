...
| 01.09 | in_progress | Draft [PR #23](https://github.com/cuneytbozok/finpill/pull/23); local checks and both Vercel Preview builds passed. Hosted API access and isolation proof remain. See handoff. |
| 01.10 | pending | — |
...
- Hosted state: Chrome shows the existing Finpill client project, Ready production at `finpill.vercel.app` from `c26804d`, and a Ready task-branch client Preview from `73cbd9f`; that Preview still has API/auth disabled. The separate `finpill-api` Vercel project (`prj_RPrp1YttJigzDOBXIUI3cYGbBjJb`) is connected to the repository, configured with Root Directory `apps/api`, outside-root files and workspace install/build commands. `APP_ENV=staging` and exact client task-branch `CLIENT_ORIGINS` are saved for API Preview only. Git-triggered commit `5241aff` deployed Ready as Preview `9btckQeTKBS25gipH2D2GBm6D9mt` at `https://finpill-api-git-codex-0109-dep-608ea4-cuneyts-projects-ec980692.vercel.app`. A direct unauthenticated `/api/v1/health` request returned `302` to Vercel SSO, so readiness does not establish application health or native reachability. The exact Preview-domain protection exception needs owner confirmation. API Production has no application variables or successful deployment. A manually requested branch deployment was unexpectedly classified as Production and ended in Build Failed; it supplies no staging proof. Production branch tracking is `main`, while Preview branch tracking is enabled for other branches. The Vercel connector returns 403 for the team, while authenticated Chrome is accessible. Draft [PR #23](https://github.com/cuneytbozok/finpill/pull/23) holds the task changes.
- Approval boundary: automatic approval review rejected saving provisional API Production settings because pilot resources and production origins have not been verified. It also rejected the exact API Preview-domain protection exception: the owner delegated the technical choice but did not explicitly approve removing Vercel Authentication. Both unsaved changes were discarded; Preview protection remains enabled. Obtain explicit approval for that specific security change before retrying it. Verify pilot prerequisites and obtain approval before configuring Production.
- Remaining task acceptance: resolve Vercel Preview access and confirm deployed `/api/v1/health`, set exact client Preview API origin, add staging-only credentials when authorized, test allowed/denied CORS and bearer requests, run an API-enabled native profile against that deployed v1 API, and inspect artifacts for server secrets. A separate pilot Supabase project/live Clerk instance are not yet registered, so Preview-to-pilot isolation needs explicit hosted evidence before acceptance. Keep A01–A03 draft and do not select 01.10 yet.
