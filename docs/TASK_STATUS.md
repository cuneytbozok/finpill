...
**State:** blocked — implementation, Native API configuration, and build checks passed; signed-device lifecycle acceptance is outstanding. See handoff below.
...
| 01.04 | blocked | iOS SDK bridge, Native API registration, and Release simulator build verified; device provisioning and live lifecycle checks remain. See handoff. |
| 01.05 | pending | — |
| 01.06 | pending | — |
| 01.07 | pending | — |
...
- Blocked live acceptance: the owner selected Personal Team `7NA7D47449` for Debug and Release in Xcode with automatic signing; Codex did not edit signing settings. A signed generic iOS Release build with `-allowProvisioningUpdates` failed because the team has no registered physical device and Xcode could not create a provisioning profile. The owner can connect an iPhone; signed-device testing is pending. Release-mode sign-in, token refresh, secure cold restart, expiry, network loss/recovery, resume, sign-out and token-leakage checks were not performed. Cancellation was checked only on the unsigned simulator. No A01–A03 gate is accepted.
- Next action: connect/trust a physical iPhone, let Xcode provision this App target, then run the guide's live acceptance in this same worktree/branch. Preserve the 01.02 iOS offline waiver as waived, not passed. Do not start another roadmap task in this implementation session.
