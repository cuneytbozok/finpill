...
The root `vercel.json` deploys the static client only: frozen installation with `npm ci`, `npm run build:client`, and output `apps/client/out`. Keep this project's Root Directory at the repository root (blank in Vercel). The explicit output overrides the generic `public` default; the framework is `null` (Other) because Vercel serves the exported assets rather than a Next.js server. Its SPA rewrite sends canonical deep links to `index.html`, where the shared route registry resolves the path. Other web hosts and future native shells must provide the equivalent entry-asset fallback; a static export cannot prebuild arbitrary tickers.

The API needs a separate Vercel project rooted at `apps/api`, using the Next.js preset and access to shared workspace files. Do not point that project at the root client configuration or deploy `apps/api/.next` as static files. Coordinated client/API deployments, a credential-free Preview and fail-closed Production configuration remain task 01.09 (see [ENVIRONMENTS.md](ENVIRONMENTS.md)). This preview configuration does not establish native routing, authentication, or API deployment readiness.

Task 00.04 owns validated environment configuration. Task 00.05 adds [local database setup and CI](DATABASE_DEVELOPMENT.md). Task 01.01 provides the shared canonical route registry, browser navigation adapter, minimal `AuthPort`, and validated API transport. The static shell never requests the API while it is disabled; enabling it requires the validated origin from the environment reader. Tasks 01.02–01.10 prove native/auth deployment behavior. This placeholder page is not a product screen, and the static build alone does not prove Capacitor routing or authentication.
