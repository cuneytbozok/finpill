...
| BC-21 | §6–9, §12.4–12.8, §34 | Correct the KAP source description with the observed MKK VYK responses recorded in [KAP_FIXTURES.md](KAP_FIXTURES.md): `rounding` is a precision indicator on already-scaled values (`INF`, `-3`, `-6`), never a multiplier; eight FR taxonomy families with differing role sets (not four fixed `general_role` IDs); dimensional facts such as the banks' `CurrencyTypeAxis` totals; `presentation` also carries non-numeric form values; corporate actions and several notices arrive as `flatData` (serialized Java DTOs), and `fileType=html` as base64 `htmlMessages`; `disclosureReason` (`NEW`/`UPD`/`CORR`) with `relatedDisclosureIndex` links corrections; attachments are Java-serialized byte arrays wrapping the PDF; errors are HTTP 400 with inconsistent bodies; the API removes personal-data disclosures/attachments (`blockedDisclosures`); by owner decision, copies already acquired are retained because they were publicly published (A05 records the handling). | 02.02–02.05, 03.01–03.04, 04.01–04.06 | 02.01 evidence and owner decision on takedowns (2026-09-25) | recorded |
