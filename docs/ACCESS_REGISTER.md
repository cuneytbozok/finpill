...
| iOS local identity and signing | Project owner | Bundle ID `com.cuneytbozok.finpill`; local Xcode/Personal Team development signing where needed | owner-confirmed plan; runtime unverified | Owner confirmed personal/private use on 2026-09-23. Xcode project, simulator/device build and signing behavior still require task evidence. | 01.02 and 01.04. Use local development signing only when the chosen test target needs it; verify bundled-asset cold start and inspect artifacts. |
