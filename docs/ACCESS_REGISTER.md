...
| iOS local identity and signing | Project owner | Bundle ID `com.cuneytbozok.finpill`; local Xcode/Personal Team development signing where needed | simulator build confirmed; device team unassigned | Task 01.04's Xcode inspection found `CODE_SIGN_STYLE=Automatic`, bundle ID `com.cuneytbozok.finpill`, and no `DEVELOPMENT_TEAM` in the App target or resolved build settings. The Release simulator build passed unsigned. The owner reports a Personal Team in Xcode, but the App target's team and physical-device signing remain unverified. | 01.04. Select the intended team in Xcode for device testing and use its App ID Prefix to register the Clerk native application; do not assume simulator build proves device signing. |
