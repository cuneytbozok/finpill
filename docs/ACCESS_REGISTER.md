...
| Clerk | Project owner | Test/staging/production instance IDs: not recorded | unconfirmed | No Clerk instance, invitation policy, JWT issuer, or keys are configured. | 01.03–01.06. Establish separate non-production and pilot instances, then record owner and non-secret identifiers. |
...
| iOS signing | Project owner | Apple Developer team, bundle ID, certificates/profiles: not recorded | unconfirmed | No signing identity or native project is present. | 01.02, 01.04, 01.09. Confirm team owner and release signing custody without storing material in the repository. |
| Android signing | Project owner | Play Console account, application ID, keystore custody: not recorded | unconfirmed | No signing identity or native project is present. | 01.02, 01.05, 01.09. Confirm account owner and keystore custody without storing keys in the repository. |
