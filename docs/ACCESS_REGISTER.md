...
| Vercel client project | Project owner | `finpill` → Production `finpill.vercel.app` (static client) | confirmed | 01.09: Preview build on 2026-09-24 reported `CLERK_SECRET_KEY`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `NEXT_PUBLIC_API_ENABLED` in Preview scope. Remove them and rotate the Clerk development secret. Client-scope Supabase entries were deleted during 01.06 with owner approval. |
| Vercel API project | Project owner | `finpill-api` (`prj_RPrp1YttJigzDOBXIUI3cYGbBjJb`), Root Directory `apps/api` | created during PR #23; Production not deployed | 01.09: remove the Preview `APP_ENV` variable (the Preview build on 2026-09-24 rejected it), re-evaluate the exact task-branch Preview domain's Vercel Authentication exception, and configure Production fail-closed (auth off until Clerk live). |
...
| KAP / MKK | Project owner | Development/production accounts and endpoints not recorded | unconfirmed | 03.01–03.07: confirm owner, permitted endpoint, rate plan and retention terms. |
...
| KAP financial-report fixtures | Project owner | Permitted source set and manifest not selected | unconfirmed | 02.01, 03.01, 04.02–04.03: select a permitted cohort and record retrieval time, hash, origin and rights. |
